...
getGrandTotal = () =>{
    ...
    if (totalinventory != null && totalinventory.length > 0) {
        ...
        for (let shop_owner = 0; shop_owner < totalinventory.length; shop_owner++) {

            grandTotal  += parseFloat(totalinventory[shop_owner]["total"]);
...
showInvent = () =>{
    ...
    if (totalinventory != null && totalinventory.length > 0) {
        ...
        for (let shop_owner = 0; shop_owner < totalinventory.length; shop_owner++) {
            ...
            inventoryProduct.innerHTML = totalinventory[shop_owner]["product"];
            inventoryPrice.innerHTML = totalinventory[shop_owner]["price"];
            inventoryQuantity.innerHTML = totalinventory[shop_owner]["quantity"];
            inventoryTotal.innerHTML = totalinventory[shop_owner]["total"];
            ...
            btn.className = "btn";
            btn.value = "delete";
            btn.onclick = (function(shop_owner) {
                return function() {
                    ...
                    if (confirm("Do you want to delete your inventory data ?")) {
                        ...
                        totalinventory.splice(shop_owner, 1) 
                        alert("item deleted")
                        window.location.reload();
                        localStorage.setItem("totalinventory", JSON.stringify(totalinventory)); 
                        getGrandTotal();
